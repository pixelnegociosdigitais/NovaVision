...
async function run() {
  ...
  const res = await fetch('https://api.brasil.io/v1/dataset/socios-brasil/empresas/data/?uf=RS', {
    headers: { 'Authorization': `Token ${t}` }
  });
  
  const data = await res.json();
  console.log(res.status, Object.keys(data), data.results ? data.results[0] : data);
}
